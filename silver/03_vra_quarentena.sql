...
COMMENT 'Silver quarentena - espelho DIAGNOSTICO dos registros de silver.vra que reprovaram em alguma expectation do contrato de dados, com o motivo por registro.'
AS
SELECT
  *,
  ARRAY_JOIN(
    ARRAY_COMPACT(ARRAY(
      CASE WHEN NOT (partida_prevista IS NOT NULL AND chegada_prevista IS NOT NULL) 
           THEN 'horarios_previstos_presentes' END,
      CASE WHEN NOT (situacao_voo IN ('REALIZADO', 'CANCELADO')) 
           THEN 'situacao_voo_conhecida' END,
      CASE WHEN NOT (partida_prevista IS NULL OR chegada_prevista IS NULL OR chegada_prevista > partida_prevista) 
           THEN 'chegada_prevista_depois_da_partida_prevista' END,
      CASE WHEN NOT (partida_real IS NULL OR chegada_real IS NULL OR chegada_real > partida_real) 
           THEN 'chegada_real_depois_da_partida_real' END,
      CASE WHEN NOT (atraso_partida_min IS NULL OR atraso_partida_min BETWEEN -120 AND 1440) 
           THEN 'atraso_partida_plausivel' END,
      CASE WHEN NOT (atraso_chegada_min IS NULL OR atraso_chegada_min BETWEEN -120 AND 1440) 
           THEN 'atraso_chegada_plausivel' END,
      CASE WHEN NOT empresa_no_cadastro 
           THEN 'empresa_no_cadastro_anac' END,
      CASE WHEN NOT origem_no_cadastro 
           THEN 'aeroporto_origem_no_cadastro_anac' END,
      CASE WHEN NOT destino_no_cadastro 
           THEN 'aeroporto_destino_no_cadastro_anac' END
    )), 
    ' | '
